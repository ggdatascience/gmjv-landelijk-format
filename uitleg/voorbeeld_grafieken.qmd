...
```{r, fig.alt= ggplot2::get_alt_text(plot)}
#Staafdiagram met dubbele uitsplitsing, eigen titel, kleuren, custom alt-text, nvar en ncel
plot = monitor_df %>% 
  maak_staafdiagram_dubbele_uitsplitsing(
    var_inhoud = "LVEES404",
    var_crossing_groep = "AGGSA402",
...
-   **var_inhoud_waarden:** Een numeric vector waar de numerieke waarden voor antwoorden ingevuld kunnen worden. Als deze waarde leeg is worden alle antwoorden getoond. Anders alleen de antwoorden die zijn ingevuld.
...
### Aangepast staafdiagram met meerdere staven: Antwoorden filteren

Het is ook mogelijk om antwoorden weg te filteren uit maak_staafdiagram_meerdere_staven(). Dit kan met de parameter **var_inhoud_waarden**
Let op: Als je waarden invoert die niet bestaan zullen die zonder foutmelding genegeerd worden. 
